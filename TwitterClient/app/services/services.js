angular.module('twitterApp.services', []).factory('twitterService', function($q) {
    ...
    return {
        ...
        saveTweet: function (msg) {
            //create a deferred object using Angular's $q service
            var deferred = $q.defer();
            var url='/1.1/statuses/update.json';
            var promise = authorizationResult.post(url, {data: {status: msg}}).done(function(data) { //https://dev.twitter.com/docs/api/1.1/get/statuses/home_timeline
                //when the data is retrieved resolve the deferred object
                //$scope.getLatestTweets(10);
